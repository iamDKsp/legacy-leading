...
const InstagramVideo = () => {
    ...
    const [showPlayControls, setShowPlayControls] = useState(true);
    ...
    // Timer para esconder pause/play
    const resetHideTimer = useCallback(() => {
        setShowPlayControls(true);
        if (hideTimer.current) clearTimeout(hideTimer.current);
        hideTimer.current = setTimeout(() => setShowPlayControls(false), CONTROLS_HIDE_DELAY);
    }, []);
    ...
    // Aplica muted ao carregar novo vídeo
    useEffect(() => {
    ...
    // Mute: manipula DIRETAMENTE a DOM property — sem dependência de React props
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black">
                ...
                {/* ── BOTÕES FIXOS (mute + instagram) — sempre visíveis durante play ── */}
                {isPlaying && (
                    <div className="absolute top-3 left-3 right-3 flex justify-between items-center z-30">
                        {/* Mute — completamente fora do AnimatePresence */}
                        <button
                            onClick={handleMuteClick}
                            className="p-2 bg-black/70 rounded-full text-white hover:bg-[#C5A572] hover:text-black backdrop-blur-sm transition-colors"
                        ...
                        <a
                            ...
                            className="p-2 bg-black/70 rounded-full text-white hover:bg-[#C5A572] hover:text-black backdrop-blur-sm transition-colors flex items-center"
                ...
                <AnimatePresence mode="wait">
                    {!isPlaying ? (
                        ...
                        >
                            ...
                            {/* Pause/Play + setas — somem após 3s de inatividade */}
                            <AnimatePresence>
                                {showPlayControls && (
                                    <motion.div
                                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
